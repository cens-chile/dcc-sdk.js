...
export async function makeCWTDVC(payload, monthsToExpire, issuer) {
  const CWT_ISSUED_AT = 6;
  const CWT_EXPIRATION = 4;
  const CWT_ISSUER = 1;
  const CWT_HCERT_V1 = -6;
  const CWT_HCERT_DDVC = -260;
  let cwt = new Map();
  ...
  cwt.set(CWT_HCERT_DDVC, new Map()); 
  cwt.get(CWT_HCERT_DDVC).set(CWT_HCERT_V1, payload);
  return cwt;
}

export async function makeCWTDDVC(payload, monthsToExpire, issuer) {
  const CWT_ISSUED_AT = 6;
  const CWT_EXPIRATION = 4;
  const CWT_ISSUER = 1;
  const CWT_HCERT_V1 = 2;
  const CWT_HCERT_DDVC = -260;
  let cwt = new Map();
...
export async function parseCWTDVC(cwt) {
  if (cwt.get(CWT_HCERT))
    return cwt.get(CWT_HCERT).get(-6);
  else 
